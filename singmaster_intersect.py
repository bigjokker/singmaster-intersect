...
def fib_member(i: int, compute_m: bool = True) -> FibMember:
    ...
    if compute_m:
        t0 = time.time()
        mem.m = comb(n, k)
        mem.m_digits = int(mem.m.bit_length() * math.log10(2.0)) + 1
        dt = time.time() - t0
        print(
...
def pair_is_past_2017(n: int, k: int, m: int, l: int, value: mpz) -> bool:
    ...
    if max(n, m) <= ROW_BOUND_2017:
        return False
    digits = int(value.bit_length() * math.log10(2.0)) + 1
...
def hit_from_value(
    ...
    digits = int(m.bit_length() * math.log10(2.0)) + 1
...
def run_nearby(
    ...
    for chunk in raw_chunks:
        ...
        for d, e, n, k, status, is_fib in chunk:
            l = k + e
            mrow = n - d
            val = comb(n, k)
            ...
            digits = int(val.bit_length() * math.log10(2.0)) + 1
            if digits <= 5000:
                extra = extra_reps(
                    val,
...
def run_collide(
    ...
    d_lo = int(sample_lo.bit_length() * math.log10(2.0)) + 1
    d_hi = int(sample_hi.bit_length() * math.log10(2.0)) + 1
