...
def cells() -> list[dict]:
    ...
    for a in range(2, amax + 1):
        for b in range(1, a):
            plo = max(N // (a + 1) + 1, K // (b + 1) + 1)
            phi = min(N // a, K // b)
            if plo > phi or phi < 3287 or plo > K:
                continue
            s = a - b
            zlo = D // s + 1
            if zlo <= plo:
...
def live_intervals(windows: list[dict]) -> list[tuple[int, int]]:
    ivs = []
    for w in windows:
        if w["kind"] == "NONE":
...
def first_live_after(x: int, ivs: list[tuple[int, int]]) -> int | None:
...
def live_primes(k: int, n: int, ivs: list[tuple[int, int]]) -> list[int]:
    out = []
    x = k
    for _ in range(n):
...
def summarize(rows: list[dict]) -> dict:
    n = len(rows)
    if not n:
        return {"n": 0, "even": None, "mean_k": None, "mean_g": None}
    even = sum(1 for s in rows if s.get("g_even")) / n
    mean_k = sum(s["k"] for s in rows) / n
    mean_g = sum(s["g"] for s in rows) / n
    return {
        "n": n,
        "even": round(even, 4),
        "mean_k": round(mean_k),
