...
def preflight() -> None:
    ...
    assert N == 10_803_704 and K == 4_126_647
    assert int(fib(16) * fib(19)) == K
    assert int(fib(18) * fib(17)) == K + 1
    assert K != int(fib(18) * fib(17))
    assert D == 6_677_057 and KMAX == 5_182_637 and N2 == 5_401_852
    assert log10_central(KMAX) <= LOG10_M < log10_central(KMAX + 1)
    assert KMAX - (K + 2) + 1 == 1_055_989 == NCOLS
    ...
    for p in PRIMES:
        ...
        assert 2 * p > N and p <= D and p > KMAX
...
def summarize(surv: list[dict]) -> dict:
    ...
    even = sum(1 for s in surv if s["g_even"]) / n
    mean_k = sum(s["k"] for s in surv) / n
    return {"n": n, "even": round(even, 4), "mean_k": round(mean_k)}
